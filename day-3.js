...
const getCombinedCoords = commands => {
  return commands.reduce((combinedCoords, command) => {
    ...
    return [...combinedCoords, ...getCoords(command, { x, y })]
  }, []);
};
...
const getClosest = crosses => Math.min(...crosses.map(cross => cross.split(DELIMITER).reduce((a, c) => a + Math.abs(Number(c)), 0)));
...
const manhattan = (wire1, wire2) => {
  ...
  console.log(getClosest(crosses));
